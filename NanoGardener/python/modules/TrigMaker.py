...
class TrigMaker(Module):
    ...
    def __init__(self, cmssw = 'Full2016', isData = False, keepRunP = False, cfg_path = 'LatinoAnalysis/NanoGardener/python/data/TrigMaker_cfg.py', seeded = False, branch_map='', isFastSim = False):
        self.cmssw = cmssw
        self.isData = isData
    ...
    def initReaders(self): # this function gets the pointers to Value and ArrayReaders and sets them in the C++ worker class
        ...
        for RunP in self.Trigger[self.cmssw]:
           ...
           if RunP<firstPeriod: firstPeriod = RunP
           self.TM_trig[RunP]    = {}
           self.TM_LegEff[RunP]  = {}
           self.TM_DZEff[RunP]   = {}
           self.TM_GlEff[RunP] = {}
        ...
        if self.isData:
         ...
         for iPeriod in range(1,len(self.RunFrac)) :
           if toss_a_coin >= self.RunFrac[iPeriod-1] and toss_a_coin < self.RunFrac[iPeriod]:
              return iPeriod+firstPeriod-1
           if toss_a_coin == 1.0:
              return len(self.RunFrac)-1+firstPeriod-1
